...
var handler = func(w http.ResponseWriter, r *http.Request) {
	...
	for _, dbReceipt := range dbReceipts {
		...
		if !ok {
			...
			receipt.Categories = make([]Category, 0)
		}

		isCategoryUnique := true
		for _, category := range receipt.Categories {
			if category.Name == dbReceipt.CategoryName {
		...
		if isCategoryUnique && dbReceipt.CategoryName != "" {
			receipt.Categories = append(
				receipt.Categories,
				Category{
					ID:    dbReceipt.CategoryId,
					Name:  dbReceipt.CategoryName,
					Color: dbReceipt.CategoryColor,
				},
			)
		}
	...
	for _, dbReceipt := range dbReceipts {
		receipt := receiptsMap[dbReceipt.ReceiptId]
		date := dbReceipt.Date.Format("2006-01-02")
	...
	for _, date := range dates {
		receipts := aggregatedByDate[date]

		total := 0
		for _, receipt := range receipts {
			var dbReceipt ReceiptDB
